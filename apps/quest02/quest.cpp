...
namespace Calc {
  ...
  bool QuestAppOptions::parseOptions(int argc, char* argv[]){
    ...
    if(m_input.filename.empty())
    {
      switch(m_algo.type)
      {
        case A_NumCppUniform:
          m_input.filename = "uniform";
          break;
        case A_NumCppChebyshev:
        default:
          m_input.filename = "chebyshev";
          break;
      }
    }
    if(m_output.filename.empty())
    {
      ...
      {
        case A_NumCppUniform:
          m_output.filename = "res_uniform";
          break;
        case A_NumCppChebyshev:
        default:
          m_output.filename = "res_chebyshev";
          break;
      }
    }
    return result;
  ...
  void QuestApp::setDefaultOptions()
  {
    ...
    m_input.filetype = _input_opt_names[0].type; m_input.filename = "chebyshev";
    m_output.filetype = _output_opt_names[0].type; m_output.filename = "res_chebyshev";
#ifdef QUESTAPP_OPT_DEFAULT_ALGO
  ...
  void QuestApp::run()
  {
    //PRERUN:
    log().debug(Summary());
    TInterpolantType interpolant_type = TInterpolantType::Array;
    switch(m_algo.type)
    {
#ifdef HAVE_GSL
      case A_ExtGSLUniform :
      case A_ExtGSLChebyshev :
        interpolant_type = TInterpolantType::GSL;
#endif
      default:
        interpolant_type = TInterpolantType::Array;
        break;
    }
    TInterpolantFlavour interpolant_flavour = TInterpolantFlavour::Uniform;
    switch (m_algo.type) {
      case A_NumCppUniform:
#ifdef HAVE_GSL
      case A_ExtGSLUniform:
#endif
        interpolant_flavour = TInterpolantFlavour::Uniform;
        break;
      case A_NumCppChebyshev:
#ifdef HAVE_GSL
      case A_ExtGSLChebyshev:
#endif
        interpolant_flavour = TInterpolantFlavour::ChebyshevFirstKind;
        break;
      default:
        interpolant_flavour = TInterpolantFlavour::Uniform;
        break;
    ...
    m_pAlgoParameters->f.reset(NewInterpolant1d(m_precision.type,
          m_pfIn.get(), false, interpolant_type, interpolant_flavour));
    log().fdebug("found %zu function values on interval [ %g ; %g ]",
        m_pAlgoParameters->f->getPointsCount(),
        m_pAlgoParameters->f->getRoundedLowerBorder(), m_pAlgoParameters->f->getRoundedUpperBorder());
    ...
    //TODO: check if we should cache table of interpolated values via some cli option
    m_pAlgoParameters->f->preallocateInterpolatedTable(m_pAlgoParameters->table_size);
    ...
    interpolate::perform(*m_pAlgoParameters,log());
    //output results
