...
namespace Calc {
  ...
  void QuestApp::setDefaultOptions()
  {
    ...
    m_pAlgoParameters.reset(new dense_linear_solve::AlgoParameters({m_threading,m_precision,m_algo,
          nullptr,nullptr,nullptr,0,ctrl()}));
  ...
  const std::string QuestApp::Summary() const
  {
    ...
    for ( int i = 0; _algo_opt_names[i].name; ++i )
    {
      ...
      {
        s.append("Running dense linear system solver ").append(_algo_opt_names[i].name);
        break;
      }
    }
  ...
  void QuestApp::readInput()
  {
    ...
    for(size_t i = 0; i < sz; i++)
    {
      ...
      m_pAlgoParameters->Ab_rows[i] = buf_ptr+i*stride;
    }
    for(size_t i = 0; i < sz; i++)
    {
  ...
  void QuestApp::run()
  {
    ...
    m_pAlgoParameters->Ab_rows = new double* [m_pAlgoParameters->system_size];
    ...
    //log stats
    delete[] m_pAlgoParameters->Ab_rows;
